-- Revert buzz_start_time back to Buzz_start_time (only if lowercase column exists)
DO $$ BEGIN
    IF EXISTS (
...
-- Revert buzz_end_time back to Buzz_end_time (only if lowercase column exists)
DO $$ BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' 
