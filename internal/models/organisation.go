...
type Organisation struct {
	...
	CurrentUserRoleInfo OrgUserRoleInfo  `gorm:"-" json:"current_user_role_info,omitempty"`
}
...
func (o *Organisation) AddSystemAgentstoOrg(db *gorm.DB, logger *utility.Logger) error {
	...
	err := db.Model(&Integrations{}).
		Select("gen_random_uuid() AS id, id as integration_id, ? as org_id, ? as owner_id, name as app_name, app_description, app_url, app_logo, json_url, false as is_active, true as is_system, NOW() as created_at, NOW() as updated_at, system_prompts, tone, title, skills, category, capabilities, visibility, stars, prices, provider, default_input_modes, default_output_modes, version, commission_rate", o.ID, o.OwnerID).
		Where("app_logo != '' AND system_prompts IS NOT NULL").
		Limit(5).
