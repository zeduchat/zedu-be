...
func TestGetUserNotificationPreferences(t *testing.T) {
	...
	t.Run("Successful Get User Notification Preferences", func(t *testing.T) {
		...
		tests.AssertResponseMessage(t, response["message"].(string), "User notification preferences retrieved successfully")
	})

	t.Run("Successful Get User Notification Preferences for regular user", func(t *testing.T) {
		router, authController := setup()
		loginData := models.LoginRequestModel{
