...
func SetupThreadsRoutes(r *gin.Engine, threadController *thread.Controller) {
	...
	r.PUT("/api/v1/threads/:thread_id/users/:user_id", middleware.Authorize(threadController.Db.Postgresql),
		threadController.UpdateAThread)
	r.GET("/api/v1/threads/:thread_id/users/:user_id", middleware.Authorize(threadController.Db.Postgresql),
		threadController.GetUserSingleThreads)
	r.GET("/api/v1/threads/users/:user_id/channels/:channel_id", middleware.Authorize(threadController.Db.Postgresql),
		threadController.GetAllUserChannelThreads)
