...
func TestCreateBlogCategory(t *testing.T) {
	...
	name := "tomiwa" + utility.GenerateUUID()

	tests := []struct {
		Name         string
		RequestBody  models.BlogCategoryCreateReq
		ExpectedCode int
		Message      string
		Headers      map[string]string
	}{
		{
			Name: "Successful Creation of Blog Category",
			RequestBody: models.BlogCategoryCreateReq{
				Name: name,
			},
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			req, _ := http.NewRequest(http.MethodPost, "/api/v1/blog_categories", &b)
