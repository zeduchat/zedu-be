...
func User(r *gin.Engine, ApiVersion string, validator *validator.Validate, db *storage.Database, logger *utility.Logger) *gin.Engine {
	...
	adminUrl := r.Group(fmt.Sprintf("%v", ApiVersion), middleware.Authorize(db.Postgresql))
	{
		...
		userUrl.DELETE("/users/me", user.DeactivateSelfUser)
		...
		userUrl.POST("/users/:user_id/status", middleware.CheckIsDeactivated(db.Postgresql), user.SetUserStatus)
		...
		userUrl.GET("/users/switch-org/:slug", middleware.CheckIsDeactivated(db.Postgresql), user.SwitchUserOrgBySlug)
		...
		userUrl.GET("/users/media-preferences", user.GetMediaPreferences)
		userUrl.PUT("/users/media-preferences", user.UpdateMediaPreferences)
		...
		userUrl.PUT("/users/voip-push-token", user.UpdateVoIPToken)
