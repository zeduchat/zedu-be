...
const recordingUID = "recording_bot"
...
func StartBuzzRecording(db *storage.Database, logger *utility.Logger, buzzID, hostID string) (*models.BuzzRecording, int, error) {
	...
	resourceID, err := agora.AcquireRecording(buzzID, recordingUID)
	if err != nil {
	...
	sid, err := agora.StartRecording(resourceID, buzzID, recordingUID, durationSecs)
	if err != nil {
...
func StopBuzzRecording(db *storage.Database, logger *utility.Logger, buzzID, hostID string) (*models.BuzzRecording, int, error) {
	...
	if err := agora.StopRecording(rec.ResourceID, rec.Sid, buzzID, recordingUID); err != nil {
	...
	if err := saveRecordingAsOrgFile(db.Postgresql, logger, rec, buzz); err != nil {
		logger.Error("failed to save recording as org file: %v", err)
	}
...
func CheckRecordingStatus(db *storage.Database, logger *utility.Logger, buzzID, userID string) (*models.BuzzRecording, int, error) {
	...
	if statusStr == models.RecordingStatusStopped && rec.FileID == nil {
		now := time.Now().UTC()
		if rec.EndedAt == nil {
			rec.EndedAt = &now
		}
		if err := saveRecordingAsOrgFile(db.Postgresql, logger, rec, &buzz); err != nil {
			logger.Error("failed to save recording file on status check: %v", err)
		}
	}
...
func saveRecordingAsOrgFile(db *gorm.DB, logger *utility.Logger, rec *models.BuzzRecording, buzz *models.Buzz) error {
	...
	rec.FileID = &file.ID
	return db.Model(rec).Update("file_id", file.ID).Error
}
...
func publishRecordingEvent(logger *utility.Logger, buzz *models.Buzz, rec *models.BuzzRecording, eventName string) {
	...
	if err := centrifuge.PublishChannel(logger, publishChannel, payload); err != nil {
		logger.Error("failed to publish %s event for buzz %s: %v", eventName, buzz.ID, err)
	}
}
...
func StopActiveRecordingForBuzz(db *storage.Database, logger *utility.Logger, buzzID string, buzz *models.Buzz) {
	...
	if err := agora.StopRecording(rec.ResourceID, rec.Sid, buzzID, recordingUID); err != nil {
		logger.Error("failed to stop agora recording on buzz end for buzz %s: %v", buzzID, err)
	}
	...
	if err := saveRecordingAsOrgFile(db.Postgresql, logger, rec, buzz); err != nil {
		logger.Error("failed to save recording as org file on buzz end: %v", err)
	}

	publishRecordingEvent(logger, buzz, rec, "recording_stopped")
	logger.Info("recording auto-stopped for buzz %s on buzz end", buzzID)
}
